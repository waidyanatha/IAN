...
if error_count == 0:
    log.append(error_count, "Starting function call load_data_from_source() in datafilter.py from main.py.")
    error_count, filename = dafi.load_data_from_source()
    if error_count > 0 or not filename :
        log.append(error_count, "Function call load_data_from_source() in datafilter.py completed. Error count: " + str(error_count) + " file name: " + str(filename))
    # else log.append the same message in previous row
else:
    log.append(error_count, "Skipped function call load_data_from_source() in datafilter.py from main.py. Error count: " + str(error_count) + " > 0.")
#
#######################################################
...
#
if error_count > 0:
    print("ERROR encountered, check the log file " + str(conf.log_file) + " with Errors " + str(error_count))
    log.append(error_count, "ERROR encountered, check the log file: " + str(conf.log_file) + " with Errors " + str(error_count))
else:
    print("SUCCESSFULLY finished the GRAB Analytics procedures " + str(tfinish))
    log.append(error_count, "SUCCESSFULLY finished the GRAB Analytics procedures.")
#
